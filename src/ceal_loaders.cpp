...
namespace Ceal {
    ...
    {
        ScopedTimer timer("LoadAudioFile_Wav");
        ...
        CEAL_RETURN_RESULT(strcmp(magic, "RIFF") == 0, fileStream, Result_InvalidFormat);
        fread(&audioFile->ChunkSize, 4, 1, fileStream);             // Chunk size
        fread(&magic, 4, 1, fileStream);                            // Wave section
        CEAL_RETURN_RESULT(strcmp(magic, "WAVE") == 0, fileStream, Result_InvalidFormat);
        fread(&magic, 4, 1, fileStream);                            // FMT section
        CEAL_RETURN_RESULT(strcmp(magic, "fmt ") == 0, fileStream, Result_InvalidFormat);
        ...
        CEAL_RETURN_RESULT(strcmp(magic, "data") == 0, fileStream, Result_InvalidFormat);
